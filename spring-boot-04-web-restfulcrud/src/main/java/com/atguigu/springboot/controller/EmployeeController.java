...
public class EmployeeController {
    ...
    // 查询所有员工返回列表页面
    @GetMapping("/emps")// @RequestMapping(method = RequestMethod.GET)
...
//        return "emps/list";
        // 正确的做法是用redirect(重定向)或forward(转发)进行一次真正的请求,而不是打开一个模板引擎的页面,/指当前项目的根路径
        // redirect和forward的原理见ThymeleafViewResolver.java的createView()方法,
        //  如redirect就去看RedirectView的对象内部的renderMergedOutputModel(),会发现最终调用servlet的sendRedirect()方法
